...
class Rooms{
  ...
  create(client,name,mode='score'){if(!['score','battle','team'].includes(mode))fail('지원하지 않는 게임 모드예요.');if(this.rooms.size>=100)fail('현재 방이 가득 찼어요. 잠시 후 다시 시도해주세요.');let code;do{code=randomBytes(4).toString('hex').slice(0,6).toUpperCase();}while(this.rooms.has(code));const room={code,mode,phase:'waiting',players:[],host:null,startAt:0,endAt:0,lastTick:0,created:this.now(),round:0,winnerIds:[],winnerTeam:null};this.rooms.set(code,room);try{this.join(client,code,name);}catch(e){this.rooms.delete(code);throw e;}return room;}
  join(client,code,name){
    ...
    const red=room.players.filter(p=>p.team==='red').length,blue=room.players.filter(p=>p.team==='blue').length;
    const p={id:randomBytes(8).toString('hex'),token:token(),name:nickname,client,ready:false,team:room.mode==='team'?(red<=blue?'red':'blue'):null,game:null,ack:0,disconnectedAt:null,lastInput:-Infinity};
  ...
  setTeam(client,team){const r=client.room,p=client.player;if(!r||r.mode!=='team'||r.phase!=='waiting')fail('팀전 대기실에서만 팀을 바꿀 수 있어요.');if(!['red','blue'].includes(team))fail('팀을 다시 선택해주세요.');p.team=team;p.ready=false;}
  start(client){const r=client.room;if(!r||r.host!==client.player.id)fail('방장만 시작할 수 있어요.');if(r.phase!=='waiting'||r.players.length<2||r.players.some(p=>!p.client||!p.ready))fail('2명 이상, 모두 연결하고 준비해야 시작할 수 있어요.');if(r.mode==='team'&&!['red','blue'].every(team=>r.players.some(p=>p.team===team)))fail('빨강팀과 파랑팀에 최소 1명씩 있어야 해요.');r.phase='countdown';r.round++;r.winnerIds=[];r.winnerTeam=null;r.startAt=this.now()+this.countdown;r.endAt=r.mode==='score'?r.startAt+this.duration:0;r.lastTick=r.startAt;const seed=token();for(const [i,p] of r.players.entries()){p.game=new (r.mode==='score'?Game:BattleGame)({mode:'classic',seed});p.ack=0;p.lastInput=-Infinity;p.incoming=[];p.sent=0;p.cancelled=0;p.finishAt=null;p.targetCursor=(i+1)%r.players.length;p.lastEvent=null;}}
  input(client,msg){const r=client.room,p=client.player;if(!r||r.phase!=='playing'||msg.round!==r.round||!Number.isSafeInteger(msg.seq)||msg.seq<=p.ack||msg.seq>p.ack+1000||!ACTIONS.includes(msg.action))return;
    ...
    if(r.mode==='score'&&this.now()>=r.endAt){this.finish(r);return;}p.ack=msg.seq;if(this.now()-p.lastInput<12)return;p.lastInput=this.now();apply(p.game,msg.action);if(r.mode!=='score'){this.resolveAttacks(r);this.markDeaths(r);}
  }
  finish(r){if(r.phase==='finished')return;if(r.mode!=='score'){this.markDeaths(r);const alive=r.players.filter(p=>p.game?.status==='playing');if(r.mode==='team'){const teams=[...new Set(alive.map(p=>p.team))];r.winnerTeam=teams.length===1?teams[0]:null;r.winnerIds=r.winnerTeam?r.players.filter(p=>p.team===r.winnerTeam).map(p=>p.id):[];}else r.winnerIds=alive.map(p=>p.id);}for(const p of r.players)if(p.game?.status==='playing')p.game.end(r.mode==='score'?'time':'win');r.phase='finished';r.finishedAt=this.now();}
  ...
  target(r,p){for(let i=0;i<r.players.length;i++){const index=(p.targetCursor+i)%r.players.length,t=r.players[index];if(t!==p&&t.game?.status==='playing'&&(r.mode!=='team'||t.team!==p.team))return{player:t,next:(index+1)%r.players.length};}return null;}
  resolveAttacks(r){
    const locked=[];
  ...
  rematch(client){const r=client.room;if(!r||r.host!==client.player.id||r.phase!=='finished')fail('방장이 경기 종료 후 대기실로 돌아갈 수 있어요.');r.players=r.players.filter(p=>p.client);r.phase='waiting';r.startAt=r.endAt=0;r.winnerIds=[];r.winnerTeam=null;for(const p of r.players){p.ready=false;p.game=null;p.ack=0;}this.host(r);}
  disconnect(client,explicit=false){const r=client.room,p=client.player;if(!r||p.client!==client)return;p.client=null;p.disconnectedAt=this.now();client.room=null;client.player=null;
    if(explicit){p.token=token();p.disconnectedAt=this.now()-this.grace-1;if(p.game?.status==='playing')p.game.end('left');if(r.mode!=='score')this.markDeaths(r);}
  ...
  tick(){const now=this.now();for(const r of this.rooms.values()){
    ...
    if(r.phase==='playing'){
      const target=r.mode==='score'?Math.min(now,r.endAt):now;let delta=target-r.lastTick;r.lastTick=target;
      // Catch up gravity with short steps, bounded for a stalled server.
      delta=Math.min(1000,Math.max(0,delta));while(delta>0){const step=Math.min(16,delta);for(const p of r.players)if(p.game?.status==='playing')p.game.update(step);if(r.mode!=='score'){this.resolveAttacks(r);this.markDeaths(r);}delta-=step;}
      if(r.mode==='battle'){this.markDeaths(r);if(r.players.filter(p=>p.game?.status==='playing').length<=1)this.finish(r);}else if(r.mode==='team'){this.markDeaths(r);const aliveTeams=new Set(r.players.filter(p=>p.game?.status==='playing').map(p=>p.team));if(aliveTeams.size<=1)this.finish(r);}else if(now>=r.endAt||r.players.every(p=>p.game?.status==='over'))this.finish(r);
    }
  ...
  view(client){const r=client.room;if(!r)return null;let players=r.players.map(p=>({id:p.id,name:p.name,ready:p.ready,team:p.team,connected:!!p.client,score:p.game?.score||0,lines:p.game?.lines||0,level:p.game?.level||1,status:p.game?.status||'waiting',reason:p.game?.reason||'',board:p.game?.board||null,piece:p.game?.piece||null,incoming:(p.incoming||[]).reduce((n,b)=>n+b.lines,0),sent:p.sent||0,cancelled:p.cancelled||0,finishAt:p.finishAt??null}));
    if(r.mode!=='score'){players.sort((a,b)=>(b.finishAt??Infinity)-(a.finishAt??Infinity)||a.id.localeCompare(b.id));let rank=1;players=players.map((p,i)=>{if(i&&p.finishAt!==players[i-1].finishAt)rank=i+1;return{...p,rank};});if(r.mode==='team'&&r.phase==='finished')players=players.map(p=>({...p,rank:r.winnerTeam?(p.team===r.winnerTeam?1:2):1})).sort((a,b)=>a.rank-b.rank||a.id.localeCompare(b.id));}else players=ranking(players);
    const own=client.player,target=r.mode!=='score'&&own.game?this.target(r,own):null;
    return{type:'state',protocol:2,now:this.now(),code:r.code,mode:r.mode,phase:r.phase,round:r.round,host:r.host,you:own.id,startAt:r.startAt,endAt:r.endAt,finishedAt:r.finishedAt||0,duration:this.duration,winnerIds:r.winnerIds,winnerTeam:r.winnerTeam,players,own:snapshot(own.game),ack:own.ack,lastEvent:own.lastEvent||null,targetId:target?.player.id||null};}
}
