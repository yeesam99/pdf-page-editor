...
function createServer({roomOptions={},allowedOrigins=process.env.ALLOWED_ORIGINS||''}={}){
 ...
 const server=http.createServer((req,res)=>{let url;try{url=new URL(req.url,'http://localhost');}catch{res.writeHead(400).end();return;}
   ...
   if(url.pathname==='/health'){res.writeHead(200,{'Content-Type':'application/json'}).end(JSON.stringify({ok:true,protocol:2,modes:['score','battle','team']}));return;}
   ...
   ws.on('message',(data,binary)=>{if(binary){ws.close(1003);return;}if(Date.now()-windowAt>=1000){windowAt=Date.now();count=0;}if(++count>100){ws.close(1008,'Too many messages');return;}
     ...
     try{
       ...
       if(m.type==='create'||m.type==='join'||m.type==='resume'){
         const battleRequest=m.type==='create'?['battle','team'].includes(m.mode):['battle','team'].includes(manager.rooms.get(String(m.code).toUpperCase())?.mode);
       ...
       switch(m.type){case'ready':manager.ready(client,m.ready);break;case'team':manager.setTeam(client,m.team);break;case'start':manager.start(client);break;case'rematch':manager.rematch(client);break;
