(function () {
  ...
  function convert(){
    clearTimeout(timer);timer=null;const start=performance.now();
  ...
  function finalUpdate(){if($('live').checked)convert();else schedule();}
  ...
  function prepareBrush(){ctx.globalCompositeOperation=tool==='eraser'?'destination-out':'source-over';ctx.strokeStyle='#182033';ctx.fillStyle='#182033';ctx.lineWidth=Number($('brush').value)*(tool==='eraser'?3:1);ctx.lineCap='round';ctx.lineJoin='round';}
  function drawTo(p){
    if(!stroke)return;
    prepareBrush();
  ...
  canvas.addEventListener('pointerdown',event=>{
    ...
    if(tool==='pen'||tool==='eraser'){ctx.beginPath();ctx.arc(p.x,p.y,ctx.lineWidth/2,0,Math.PI*2);ctx.fill();schedule();}
  });
  ...
  const examples={
    cat(){
      ...
      ctx.font=`${h}px "Courier New",monospace`;ctx.fillStyle='#182033';ctx.textBaseline='alphabetic';
  ...
  document.querySelectorAll('[data-example]').forEach(b=>b.addEventListener('click',()=>{
    replaceDrawing(()=>{ctx.lineWidth=8;ctx.lineCap='round';ctx.lineJoin='round';ctx.strokeStyle='#182033';examples[b.dataset.example]();});
  ...
  $('image-file').addEventListener('change',async()=>{
    ...
    try{
      ...
      });toast('이미지를 불러왔어요. 흰 배경의 선 그림이 잘 표현돼요.');
