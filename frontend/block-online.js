(function(){
 ...
 $('game-mode').addEventListener('change',()=>{$('mode-hint').textContent=$('game-mode').value==='battle'?'줄을 지워 공격하고, 들어올 줄을 상쇄하며 끝까지 살아남아요.':'같은 블록으로 3분 동안 점수를 겨뤄요.';});
 ...
 $('copy-result').addEventListener('click',()=>{if(state?.phase==='finished')copy('몽글 블록 · '+(state.mode==='battle'?'공격 대전':'3분 점수전')+'\n'+state.players.map(p=>p.rank+'위 '+p.name+' · '+(state.mode==='battle'?p.sent+'줄 공격':p.score.toLocaleString()+'점')).join('\n'));});
 ...
 function updateUI(){
   ...
   for(const p of state.players.filter(p=>p.id!==state.you)){
     ...
     card.name.textContent=p.name;card.name.title=p.name;card.root.classList.toggle('target',battle&&p.id===state.targetId);card.score.textContent=battle?(p.sent||0)+'줄 공격':p.score.toLocaleString()+'점';card.status.textContent=!p.connected?'연결 끊김':p.status==='over'?'플레이 종료':battle?'대기 '+p.incoming+'줄 · '+p.rank+'위':p.lines+'줄 · '+p.rank+'위';card.root.classList.toggle('ended',p.status==='over');
   }
   $('result').hidden=!finished;$('rematch').hidden=!host;$('rematch').disabled=!connected;
   if(finished){const winners=state.players.filter(p=>battle?state.winnerIds.includes(p.id):p.rank===1).map(p=>p.name).join(', ');$('result-title').textContent=battle?(winners?winners+' · 최후의 생존자!':'마지막 생존자 없이 무승부!'):winners+' · '+(state.players.filter(p=>p.rank===1).length>1?'공동 1위!':'1위!');$('result-note').textContent='나는 '+me.rank+'위 · '+(battle?me.sent+'줄 공격':me.score.toLocaleString()+'점')+'. '+(host?'대기실로 돌아가 다시 시작할 수 있어요.':'방장이 대기실로 돌아가면 다시 준비해주세요.');}
   $('match-title').textContent=finished?'같이 쌓아서, 더 즐거웠어요.':battle?'빈틈을 만들고, 공격을 받아쳐요.':'각자의 속도로, 차곡차곡.';
 }
 ...
 function canPlay(){return connected&&state?.phase==='playing'&&game?.status==='playing'&&(state.mode==='battle'||serverNow()<state.endAt);}
