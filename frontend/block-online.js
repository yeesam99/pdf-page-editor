(function(){
 ...
 const colors={I:'#8dcddd',O:'#edd080',T:'#b69bd8',S:'#a0cdb0',Z:'#eaa2aa',J:'#99b2dd',L:'#eab78a',G:'#9992a5'};
 ...
 function connect(request){
   ...
   ws.addEventListener('open',()=>{if(ws!==socket)return;lastPacket=performance.now();ws.send(JSON.stringify({...request,protocol:2}));ws.send(JSON.stringify({type:'ping',sent:performance.now()}));});
 ...
 $('roster').addEventListener('click',e=>{const button=e.target.closest('[data-team]');if(button&&!button.disabled)send({type:'team',team:button.dataset.team});});
 ...
 $('copy-result').addEventListener('click',()=>{if(state?.phase==='finished'){const attack=state.mode!=='score';copy('몽글 블록 · '+(state.mode==='team'?'팀 공격 대전':state.mode==='battle'?'공격 대전':'3분 점수전')+'\n'+state.players.map(p=>(p.team?(p.team==='red'?'빨강 ':'파랑 '):'')+p.rank+'위 '+p.name+' · '+(attack?p.sent+'줄 공격':p.score.toLocaleString()+'점')).join('\n'));}});
 ...
 function receive(m){
   if(m.protocol!==2){exitRoom('대전 서버를 업데이트한 뒤 새로고침해주세요.');return;}const previous=state,changed=!state||state.round!==m.round;state=m;if(!synced)offset=m.now-performance.now();
 ...
 function updateUI(){
   if(!state)return;const me=state.players.find(p=>p.id===state.you),host=state.host===state.you,waiting=state.phase==='waiting',finished=state.phase==='finished',team=state.mode==='team',battle=state.mode!=='score';
   $('room-mode').textContent=team?'빨강 vs 파랑 · 팀 공격 대전':battle?'공격 대전 · 마지막 생존자':'3분 점수전 · 공격 없음';$('clock-label').textContent=battle?'생존 시간':'남은 시간';$('match-eyebrow').textContent=team?'RED TEAM VS BLUE TEAM':battle?'LAST STACKER STANDING':'THREE MINUTE SCORE RACE';$('battle-stats').hidden=$('battle-rules').hidden=!battle;$('incoming').textContent=me.incoming||0;$('sent').textContent=me.sent||0;$('cancelled').textContent=me.cancelled||0;$('battle-stats').classList.toggle('danger',me.incoming>0);const target=state.players.find(p=>p.id===state.targetId);$('target-name').textContent='다음 공격 대상 · '+(target?.name||'—');const event=state.lastEvent;$('attack-feedback').textContent=event&&state.now-event.at<4500?event.label+' · 상쇄 '+event.cancelled+' / 전송 '+event.sent+(event.targetName?' → '+event.targetName:''):'내 공격은 대기 중인 방해 줄부터 상쇄해요.';$('room-label').textContent=state.code;$('members').textContent=state.players.length+' / 4명';$('lobby').hidden=!waiting;$('match').hidden=waiting;
   $('ready').textContent=me.ready?'준비 완료 · 취소':'준비하기 ✓';$('ready').disabled=!connected;$('ready').setAttribute('aria-pressed',String(me.ready));
   const teamsReady=!team||['red','blue'].every(t=>state.players.some(p=>p.team===t));$('start-match').hidden=!host;$('start-match').disabled=!connected||state.players.length<2||!teamsReady||state.players.some(p=>!p.ready||!p.connected);
   $('lobby-hint').textContent=team?(teamsReady?'인원수는 달라도 괜찮아요. 모두 준비하면 시작할 수 있어요.':'빨강팀과 파랑팀에 최소 1명씩 들어가야 해요.'):host?'모두 준비되면 시작 버튼을 눌러주세요.':'준비를 마치면 방장이 경기를 시작해요.';
   const rosterKey=JSON.stringify([team,state.players.map(p=>[p.id,p.name,p.ready,p.connected,p.id===state.host,p.team])]);
   if($('roster').dataset.key!==rosterKey){$('roster').dataset.key=rosterKey;$('roster').replaceChildren();
     for(const p of state.players){const card=elem('div','player-seat'+(p.ready?' is-ready':'')+(p.team?' team-'+p.team:''));card.append(elem('span','avatar','▦'),elem('b','',p.name+(p.id===state.you?' · 나':'')),elem('small','',(!p.connected?'재접속 대기':p.ready?'준비 완료':'준비 중')+(p.id===state.host?' · 방장':'')));if(team){const pick=elem('div','team-picker');for(const t of ['red','blue']){const b=elem('button','team-button '+t,t==='red'?'빨강':'파랑');b.dataset.team=t;b.disabled=p.id!==state.you||!p.connected;b.setAttribute('aria-pressed',String(p.team===t));pick.append(b);}card.append(pick);}$('roster').append(card);}
   ...
   $('my-name').textContent=(team?(me.team==='red'?'🔴 ':'🔵 '):'')+me.name;$('my-score').textContent=me.score.toLocaleString();$('my-lines').textContent=me.lines;$('my-level').textContent=me.level;$('my-rank').textContent=team?(me.team==='red'?'빨강팀':'파랑팀'):me.rank+'위 / '+state.players.length+'명';$('my-status').textContent=me.status==='over'?'FINISHED · 관전 중':'HAPPY STACKING';
   $('ranking-title').textContent=team?(finished?'팀전 결과 ✧':'팀 현황 ✧'):finished?'최종 순위 ✧':'지금의 순위 ✧';
   const rankingKey=JSON.stringify([team,state.players.map(p=>[p.id,p.name,p.team,p.score,p.sent,p.incoming,p.rank,p.status,p.connected])]);
   if($('ranking').dataset.key!==rankingKey){$('ranking').dataset.key=rankingKey;$('ranking').replaceChildren();for(const p of state.players){const row=elem('li','rank-row'+(p.id===state.you?' me':'')+(p.team?' team-'+p.team:''));const name=elem('span','rank-name',(team?(p.team==='red'?'🔴 ':'🔵 '):'')+p.name);name.append(elem('small','',p.id===state.you?'나':!p.connected?'연결 끊김':p.status==='over'?'탈락':''));row.append(elem('span','rank-number',team?(p.status==='over'?'×':'●'):p.rank),name,elem('span','rank-score',battle?(p.sent||0)+'줄 공격':p.score.toLocaleString()));$('ranking').append(row);}}
   ...
   for(const p of state.players.filter(p=>p.id!==state.you)){
     ...
     card.name.textContent=(team?(p.team==='red'?'🔴 ':'🔵 '):'')+p.name;card.name.title=p.name;card.root.classList.toggle('team-red',p.team==='red');card.root.classList.toggle('team-blue',p.team==='blue');card.root.classList.toggle('target',battle&&p.id===state.targetId);card.score.textContent=battle?(p.sent||0)+'줄 공격':p.score.toLocaleString()+'점';card.status.textContent=!p.connected?'연결 끊김':p.status==='over'?'플레이 종료':battle?'대기 '+p.incoming+'줄'+(team?'':' · '+p.rank+'위'):p.lines+'줄 · '+p.rank+'위';card.root.classList.toggle('ended',p.status==='over');
   }
   $('result').hidden=!finished;$('rematch').hidden=!host;$('rematch').disabled=!connected;
   if(finished){const winners=state.players.filter(p=>battle?state.winnerIds.includes(p.id):p.rank===1).map(p=>p.name).join(', ');$('result-title').textContent=team?(state.winnerTeam?(state.winnerTeam==='red'?'빨강팀':'파랑팀')+' 승리!':'양 팀 동시 탈락 · 무승부!'):battle?(winners?winners+' · 최후의 생존자!':'마지막 생존자 없이 무승부!'):winners+' · '+(state.players.filter(p=>p.rank===1).length>1?'공동 1위!':'1위!');const teamResult=!state.winnerTeam?'무승부':state.winnerIds.includes(me.id)?'우리 팀 승리':'우리 팀 패배';$('result-note').textContent=(team?teamResult+' · ':'나는 '+me.rank+'위 · ')+(battle?me.sent+'줄 공격':me.score.toLocaleString()+'점')+'. '+(host?'대기실로 돌아가 다시 시작할 수 있어요.':'방장이 대기실로 돌아가면 다시 준비해주세요.');}
   $('match-title').textContent=finished?'같이 쌓아서, 더 즐거웠어요.':team?'우리 팀을 지키고, 상대 팀을 무너뜨려요.':battle?'빈틈을 만들고, 공격을 받아쳐요.':'각자의 속도로, 차곡차곡.';
 }
 ...
 function updateClock(){if(!state||state.phase==='waiting')return;const battle=state.mode!=='score',left=Math.max(0,state.endAt-serverNow()),elapsed=Math.max(0,(state.phase==='finished'?state.finishedAt:serverNow())-state.startAt);let seconds=battle?Math.floor(elapsed/1000):Math.ceil(left/1000);if(state.phase==='countdown')seconds=battle?0:180;if(state.phase==='finished'&&!battle)seconds=0;$('timer').textContent=String(Math.floor(seconds/60)).padStart(2,'0')+':'+String(seconds%60).padStart(2,'0');document.querySelector('.clock').classList.toggle('urgent',!battle&&state.phase==='playing'&&left<30000);
   let title='',note='';if(!connected){title='재연결 중';note='서버에서 경기는 계속 진행돼요';}else if(state.phase==='countdown'){title=String(Math.max(1,Math.ceil((state.startAt-serverNow())/1000)));note=battle?'마지막까지 살아남으세요!':'같은 블록, 같은 시작';}else if(state.phase==='finished'){title=battle?(state.winnerIds.includes(state.you)?'WIN!':state.winnerIds.length?'GAME OVER':'DRAW'):'GOOD GAME!';note='최종 순위를 확인하세요';}else if(game?.status==='over'){title='플레이 종료';note='친구들의 경기를 지켜봐요.';}else if(!battle&&left<=0){title='집계 중';note='서버의 최종 결과를 기다리고 있어요.';}
   $('board-overlay').hidden=!title;$('overlay-title').textContent=title;$('overlay-note').textContent=note;
 }
 function canPlay(){return connected&&state?.phase==='playing'&&game?.status==='playing'&&(state.mode!=='score'||serverNow()<state.endAt);}
