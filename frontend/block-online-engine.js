...
(function(root){
  ...
  const battle=typeof module!=='undefined'&&module.exports?require('./block-battle-core.js'):root.BlockBattleCore;
  ...
  function snapshot(game){if(!game)return null;const data={};for(const k of fields)data[k]=game[k];data.rng=game.rng.state();if(game.rules==='battle')for(const k of ['rules','lastMove','lastKick','lastAttack'])data[k]=game[k];return JSON.parse(JSON.stringify(data));}
  function restore(data){const C=data.rules==='battle'?battle.BattleGame:core.Game;const game=new C({mode:data.mode,seed:data.seed});for(const k of fields)game[k]=JSON.parse(JSON.stringify(data[k]));if(data.rules==='battle')for(const k of ['rules','lastMove','lastKick','lastAttack'])game[k]=data[k];game.rng.restore(data.rng);return game;}
